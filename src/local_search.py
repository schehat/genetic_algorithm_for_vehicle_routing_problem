...
def two_opt(ga: "GA", individual: ndarray):
    ...
    for i in range(start_index, len(best_route) - 3):
        ...
        for j in range(i + 3, len(best_route) - 1):
            ...
            total_fitness, total_distance, total_capacity_violation, total_time_warp, total_duration_violation = ga.decode_chromosome(
                new_route)
...
def two_opt_random(ga: "GA", individual: ndarray):
    """
    Applies two opt in place
    param: ga - genetic algorithm
    param: individual - structured 3D element ["individual"]["chromosome]["fitness"]
    ...
    random_range1 = list(range(start_index, len(best_route) - 3))
    random.shuffle(random_range1)

    # Until len - 3 to guarantee 3 further edges exist
    for i in random_range1:
        random_range2 = list(range(i + 3, len(best_route) - 1))
        random.shuffle(random_range2)

        # Starting from i + 3 to guarantee that i and j are not adjacent edges
        for j in random_range2:
...
def two_opt_single(ga: "GA", individual: ndarray):
    ...
    # Until len - 3 to guarantee 3 further edges exist
    for i in random_range1[:5]:
        random_range2 = list(range(i + 3, len(best_route) - 1))
        random.shuffle(random_range2)

        # Starting from i + 3 to guarantee that i and j are not adjacent edges
        for j in random_range2[:5]:
